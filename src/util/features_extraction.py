...
# Регулярные выражения для вытаскивания параметров номенклатур, разделённые по классам
FEATURES_REGEX_PATTERNS_BY_CLASSES = {
    "Бетон Материалы": {
...
_PATTERNS_BY_CLASSES_KEYS_LIST = list(FEATURES_REGEX_PATTERNS_BY_CLASSES.keys())
...
def get_noms_metadatas_with_features(df_noms_with_features: DataFrame) -> list[dict]:
    ...
    for _, row in df_noms_with_features.iterrows():
        ...
        if group_name in _PATTERNS_BY_CLASSES_KEYS_LIST:
            # Получаем набор регулярных выражений для текущей группы
            group_patterns = FEATURES_REGEX_PATTERNS_BY_CLASSES.get(group_name, {})
