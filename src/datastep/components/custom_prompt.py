...
if db_driver == "MSSQL":
    additional = "Wrap each column name in square brackets ([]) to denote them as delimited identifiers."
    # TODO: 12 и 13 строчки, кажется, не нужны. Проверить это и удалить, если не нужны. Если не нужны, убрать и упоминание MSU
...
_custom_prompt = """You are an %s expert. Given an input question, first create a syntactically correct %s query to run, then look at the results of the query and return the answer to the input question.
...
Never query for all columns from a table. You must query only the columns that are needed to answer the question. %s
...
""" % (db_driver, db_driver, additional, db_driver)
