...
from model.file import file_model, upd_file_model
...
@router.post("/extract/upd", response_model=list[str])
@version(1)
@modes_required([TenantMode.CLASSIFIER])
def extract_upd_nomenclatures(
    file_object: UploadFile,
    current_user: UserRead = Depends(get_current_user),
):
    return upd_file_model.extract_noms_from_upd(file_object)
